...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    value_0 = (mdp_data['transition_probs'][state, :, 0]).T @ mdp_data['value']
    value_1 = (mdp_data['transition_probs'][state, :, 1]).T @ mdp_data['value']
    if value_0 > value_1:
        return 0
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    mdp_data['transition_probs'] = mdp_data['transition_counts'] / np.sum(mdp_data['transition_counts'], axis=1,
                                                                          keepdims=True)
    mdp_data['transition_probs'][np.isnan(mdp_data['transition_probs'])] = 1 / mdp_data['num_states']
    mdp_data['reward'] = -mdp_data['reward_counts'][:, 0] / mdp_data['reward_counts'][:, 1]
    mdp_data['reward'] = np.nan_to_num(mdp_data['reward'])
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        for s in range(num_states):
            value_0 = (mdp_data['transition_probs'][s, :, 0]).T @ mdp_data['value']
            value_1 = (mdp_data['transition_probs'][s, :, 1]).T @ mdp_data['value']
            if value_0 < value_1:
                values[s] = mdp_data['reward'][s] + gamma * value_1
            else:
                values[s] = mdp_data['reward'][s] + gamma * value_0
        it += 1
        if np.max(np.absolute(values - mdp_data['value'])) < tolerance:
...
def main(plot=True):
    ...
    pause_time = 0.000000000000000001 * 0.000000000000000001
